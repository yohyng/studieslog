...
module.exports = async function handler(req, res) {
    ...
    const [articlesRes, settingsRes] = await Promise.all([
        ...
        fetch(`${SUPABASE_URL}/rest/v1/site_settings?id=eq.1&select=site_title,og_image,digest_bg_color,digest_text_color,digest_bg_image,digest_intro,font,title_font,font_size,line_height,letter_spacing,text_align,content_width,bg_color,text_color,body_text_opacity,title_text_opacity,footnote_color,footnote_bg_color`, { headers: sbHeaders }),
    ]);
    ...
    const messages = subscribers.map(sub => ({
        ...
        html: buildDigestHtml({
            ...
            digestIntro: settings.digest_intro || '',
        }),
    }));
