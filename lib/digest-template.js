 ...
 */
function buildDigestHtml({ siteTitle, siteUrl, articles, unsubscribeUrl, bgColor, textColor, fontFamily, typography, bgImage, digestIntro }) {
    const t = typography || {};
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${bg}"
                    ...
                    ${digestIntro ? `<tr>
                        <td style="padding-bottom: 24px;">
                            <p style="font-size:${smallSize}px; color:${text}; background-color:${bg}; line-height:1.9; margin:0; opacity:0.75;">${escapeHtml(digestIntro)}</p>
                        </td>
                    </tr>` : ''}
                    <tr>
                        <td style="padding-bottom: 24px;">
...
/**
 * 配信メールの件名を組み立てる。「essay｜プラダとナイロン 他4件」の形。
 *
 * 「今週の更新(5件)」のような定型だと、受信箱で最初に目に入る位置に中身が出てこない。
 * その週で一番読み応えのある記事(本文の文字数が最も多いもの)を選び、そのカテゴリと
 * タイトルを頭に置いて、残りは件数で添える。差出人名が「◯◯ letter」なので、
 * 件名にサイト名を重ねては字数の無駄になる。カテゴリの方が号の性格を早く伝えられる。
 *
 * @param {object} o
 * @param {string} o.siteTitle サイトタイトル。記事にタイトルが無いときの予備にだけ使う
 * @param {Array}  o.articles  [{ title, content, category }]
 */
function buildDigestSubject({ siteTitle, articles }) {
    const list = Array.isArray(articles) ? articles : [];
    if (list.length === 0) return siteTitle || 'letter';

    // essay があればその中で最長を先頭記事とする。なければ全体で最長
    const essays = list.filter(a => a.category === 'essay');
    const pool = essays.length > 0 ? essays : list;
    const lead = pool.reduce((best, a) =>
        stripTags(a.content).length > stripTags(best.content).length ? a : best);
    // カテゴリ(log / sketch / essay)は未設定の記事もあるので、その場合は区切りごと省く
    const category = lead.category ? `${lead.category}｜` : '';
    // 極端に長いタイトルでも件名が破綻しないよう頭を落とす(通常のタイトルはそのまま通る)
    const title = truncateText(lead.title || '', 60) || siteTitle || '';
    const rest = list.length - 1;
    const tail = rest > 0 ? ` 他${rest}件` : '';

    return `${category}${title}${tail}`;
}

/**
 * ダイジェストメール内の記事を表示順に並べ替える。
 * essay を最優先にし、同カテゴリグループ内は本文量の多い順。
 * send-digest / send-test-digest / 管理画面プレビューで共通利用する。
 */
function sortDigestArticles(articles) {
    return [...articles].sort((a, b) => {
        const aEssay = a.category === 'essay' ? 0 : 1;
        const bEssay = b.category === 'essay' ? 0 : 1;
        if (aEssay !== bEssay) return aEssay - bEssay;
        return stripTags(b.content).length - stripTags(a.content).length;
    });
}
...
const api = { buildDigestHtml, buildDigestSubject, buildDigestFrom, sortDigestArticles, articleBodyToEmail, escapeHtml, extractFirstImage, extractExcerpt };
