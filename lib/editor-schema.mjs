...
// ── 画像キャプション(出典明記など) ────────────────────────────────────────────
// ウェブから拾った画像の出典を明記できるようにするためのもの。通常の画像(インライン、
// 段落の中)とは別に、キャプション付きだけをブロックの<figure>として持つ。
// 理由: <figure>はHTML的にブロック要素なので、<p>の中に入れて往復させるとブラウザの
// HTMLパーサが暗黙に<p>を閉じてしまい構造が壊れる(このファイル冒頭の画像の注記と同じ問題)。
// キャプションを付ける操作は書き手の明示的な操作なので、その時だけ独立した行になる
// (段落が分割される)のは許容する。キャプション無しの画像は従来どおりImageWithStyleのまま。
export const ImageFigure = Node.create({
    ...
    addAttributes() {
        return {
            ...
            style: {
                default: null,
                parseHTML: (el) => el.querySelector('img')?.getAttribute('style') || null,
            },
            class: {
                default: null,
                parseHTML: (el) => el.querySelector('img')?.getAttribute('class') || null,
            },
            caption: {
                default: '',
    ...
    parseHTML() {
        return [{ tag: 'figure.image-figure', priority: 100 }];
    },

    renderHTML({ node }) {
        const { src, style, class: klass, caption } = node.attrs;
        const img = ['img', { src: src || '', ...(style ? { style } : {}), ...(klass ? { class: klass } : {}) }];
        return ['figure', { class: 'image-figure' }, img, ['figcaption', { class: 'image-caption' }, caption || '']];
    },
});

// ── 動画埋め込み(YouTube・Vimeo) ─────────────────────────────────────────────
// 保存形式は、16:9でレスポンシブに表示するラッパーごとそのまま持つ(book-link-cardと
// 同じ考え方で、閲覧ページ側にJSを追加しなくても表示できる自己完結したHTMLにする)。
...
export const VideoEmbed = Node.create({
    ...
    // priorityを上げないと、他のdiv/iframeを素通しする拡張があった場合に横取りされうる
    parseHTML() {
        return [{ tag: 'div.video-embed', priority: 100 }];
    },
